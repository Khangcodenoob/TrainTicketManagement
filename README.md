...
### 6. **Xác Thực & Bảo Mật (Authentication & Security)**
   - Đăng ký/Đăng nhập
...
1. Mở folder `g:\Khang De Thuong\Duong Chi Khang` trong VS Code
...
| **Nhân Viên** | staff | Staff@123 | Nhân viên |
| **Khách Hàng** | customer | Customer@123 | Khách hàng |
...
```http
GET    /api/traintrips                # Lấy tất cả chuyến
POST   /api/traintrips                # Tạo chuyến mới
GET    /api/traintrips/{id}           # Lấy chuyến theo ID
PUT    /api/traintrips/{id}           # Cập nhật chuyến
DELETE /api/traintrips/{id}           # Xóa chuyến
GET    /api/traintrips/available      # Lấy chuyến còn chỗ
```
...
GET    /api/tickets/customer/{customerId}  # Lấy vé của khách
```

### 📊 Bảng Điều Khiển

```http
GET    /api/dashboard/statistics      # Thống kê chung
GET    /api/dashboard/revenue         # Doanh thu
GET    /api/dashboard/customers       # Thống kê khách
GET    /api/dashboard/trips           # Thống kê chuyến
```

### 📋 Nhật Ký Kiểm Tra

```http
GET    /api/auditlogs                 # Lấy nhật ký
```
...
#### Controllers/
```
AuthController.cs          → Xử lý đăng nhập, đăng ký, JWT
